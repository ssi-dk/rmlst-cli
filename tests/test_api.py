...
def test_identify_success(tmp_path):
    ...
    with patch("rmlst_cli.http.call_rmlst_api", return_value=mock_response) as mock_call:
...
def test_identify_graceful_failure(tmp_path):
    ...
    with patch("rmlst_cli.http.call_rmlst_api", side_effect=http.RmlstNetworkError("Network error")):
...
def test_identify_dir_graceful(tmp_path):
    ...
    with patch("rmlst_cli.http.call_rmlst_api", side_effect=[http.RmlstNetworkError("Fail"), {"ok": True}]):
        # graceful=False -> raises
        with pytest.raises(http.RmlstNetworkError):
