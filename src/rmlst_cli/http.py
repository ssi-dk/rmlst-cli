...
DEFAULT_URI = "https://rest.pubmlst.org/db/pubmlst_rmlst_seqdef_kiosk/schemes/1/sequence"
        ...
        try:
            ...
            if response.status_code == 429 or 500 <= response.status_code < 600:
                if attempt <= retries:
                    ...
                    raise RmlstHttpError(response.status_code, response.text[:1000]) # Truncate body
...
def call_rmlst_api(
    ...
    try:
        return _make_request(session, uri, payload, retries, retry_delay)
    except (RmlstNetworkError, RmlstHttpError) as e:
        # Check if we should fallback
        if uri == DEFAULT_URI:
            try:
                return _make_request(session, FALLBACK_URI, payload, retries, retry_delay)
