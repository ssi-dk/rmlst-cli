...
def handle_exception(e: Exception, debug: bool):
    if isinstance(e, InvalidFastaError):
        ...
        print_error("more than 5000 contigs; use --trim-to-5000", EXIT_TOO_MANY_CONTIGS, debug)
    elif isinstance(e, RmlstNetworkError):
        print_error(f"network error after retries", EXIT_NETWORK_ERROR, debug)
    elif isinstance(e, RmlstHttpError):
        print_error(f"HTTP error {e.status_code} or invalid JSON", EXIT_HTTP_ERROR, debug)
...
def get_exit_code(e: Exception) -> int:
    if isinstance(e, InvalidFastaError): return EXIT_INPUT_ERROR
    if isinstance(e, TooManyContigsError): return EXIT_TOO_MANY_CONTIGS
    if isinstance(e, RmlstNetworkError): return EXIT_NETWORK_ERROR
    if isinstance(e, RmlstHttpError): return EXIT_HTTP_ERROR
    if isinstance(e, OSError): return EXIT_FS_ERROR
    return EXIT_UNEXPECTED

@click.command()
@click.option("-f", "--fasta", type=click.Path(exists=True, dir_okay=False), help="Single FASTA file input.")
@click.option("-d", "--dir", "directory", type=click.Path(exists=True, file_okay=False), help="Directory input.")
...
@click.option("--species-only", is_flag=False, flag_value="SPECIES_DEFAULT", help="Output species only.")
@click.option("--tsv", is_flag=False, flag_value="SPECIES_DEFAULT", help="Output TSV format.")
...
def main(fasta, directory, output, outdir, species_only, tsv, uri, retries, retry_delay, trim_to_5000, graceful, debug):
    ...
    try:
        if fasta:
            handle_single_file(fasta, out_path, mode, header, uri, retries, retry_delay, trim_to_5000, graceful, debug)
        else:
            handle_directory(directory, out_path, mode, header, uri, retries, retry_delay, trim_to_5000, graceful, debug)
...
def handle_single_file(fasta_path, out_path, mode, header, uri, retries, retry_delay, trim_to_5000, graceful, debug):
    final_out_path = out_path
    if out_path and os.path.isdir(out_path):
        suffix = ".json"
        if mode == "tsv": suffix = ".tsv"
        if mode == "species": suffix = ".txt" # Default for species-only
...
def handle_directory(dir_path, out_path, mode, header, uri, retries, retry_delay, trim_to_5000, graceful, debug):
    if out_path:
        if os.path.exists(out_path) and not os.path.isdir(out_path):
            click.echo("Error: Output path must be a directory in directory mode.", err=True)
            sys.exit(EXIT_INPUT_ERROR)
    ...
    for i, file_path in enumerate(files):
        ...
        try:
            ...
            if out_path:
                msg = str(e)
                if isinstance(e, InvalidFastaError): msg = "invalid FASTA or no sequences"
                elif isinstance(e, TooManyContigsError): msg = "more than 5000 contigs; use --trim-to-5000"
                elif isinstance(e, RmlstNetworkError): msg = "network error"
                elif isinstance(e, RmlstHttpError): msg = f"HTTP {e.status_code}"
    ...
    if out_path:
        click.echo(f"Done: {ok_count} ok, {failed_count} failed, {skipped_count} skipped.", err=True)
